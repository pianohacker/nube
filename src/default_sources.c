...
//> Macros
#define _SOURCE_ADD(name, init, update) _NubeSource name##_source = g_slice_new(_NubeSource); name##_source->init_func = init; name##_source->update_func = update; g_datalist_id_set_data(&available_sources, g_quark_from_static_string(#name), source) 
#define _VALUE_ALLOC(name, type) GValue *name##_value = g_slice_new(GValue); g_value_init(name##_value, type); g_datalist_id_set_data(&source->data, g_quark_from_static_string(#name), value)
